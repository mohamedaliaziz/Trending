...
export default function Peopie() {
  let media_type = 'person'
  let nums = new Array(10).fill(1).map((elem , index)=> index+1);
  console.log(nums);
  const [Peopie, setPeopie] = useState([]);
  const [isloding, setisloding] = useState(false);
  async function getTrending(page) {
    setisloding(true)
    let { data } = await axios.get(`https://api.themoviedb.org/3/person/popular?api_key=4695b36575dc31640be632966fff91fa&language=en-US&page=${page}`)
    setPeopie(data?.results)
  ...
  <div className='row mb-5'>
    {Peopie.map((item, index)=>  <div key={index} className='col-md-3'>
...
<Link className='card-body text-center text-decoration-none text-white' to={`/movieDetails/${item?.id}/${media_type}`} >
    <img src={'https://image.tmdb.org/t/p/w500'+item?.profile_path} className='w-100 card-img-top' alt="" />


    <h3 className='h6 card-text'>{item?.name} </h3>
    <div className='vote top-0 end-0 position-absolute '>{ item?.popularity?.toFixed(1)}</div>
