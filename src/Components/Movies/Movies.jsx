...
export default function Movies() {
  ...
  async function getTrending(page) {
    setisloding(true)
    let { data } = await axios.get(`https://api.themoviedb.org/3/discover/movie?api_key=4695b36575dc31640be632966fff91fa&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=true&page=${page}`)
...
<Link className='card-body text-center text-decoration-none text-white' to={`/movieDetails/${item?.id}/${media_type}`} >
    ...
    <h3 className='h6 card-text'>{item?.title} </h3>
